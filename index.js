...
async function fetchBotReply(outline) {
  const response = await openai.createCompletion({
    ...
    prompt: `Generate a short message to enthusiastically say "${outline}" sounds interesting and that you need some minutes to think about it. Mention one aspect of the sentence.
      ###
      promptTitle: Two dogs fall in love and move to Hawaii to learn to surf.
      reply: I'll need to think about that. But your idea is amazing! I love the bit about Hawaii!
      
      ###
      promptTitle: A plane crashes in the jungle and the passengers have to walk 1000km to safety.
      reply: I'll spend a few moments considering that. But I love your idea!! A disaster movie in the jungle!

      ###
      promptTitle: A group of corrupt lawyers try to send an innocent woman to jail.
      reply: Wow that is awesome! Corrupt lawyers, huh? Give me a few moments to think!
...
async function fetchSynopsis(outline){
  const response = await openai.createCompletion({
    model: 'text-davinci-003', 
    prompt: `Generate an engaging, professional and marketable movie synopsis based on the following idea: ${outline}. Add actors names in brackets after each character. Go with whichever one you think fits the role
...
async function fetchTitle(synopsis){
  const response = await openai.createCompletion({
    ...
    max_tokens: 20,
    temperature: 0.7,
    prompt: `Generate a gripping title of no more than 13 words for the a movie which has the following synopsis: ${synopsis}`
  })
  const title = response.data.choices[0].text.trim()
  document.getElementById('output-title').innerHTML = response.data.choices[0].text.trim()
...
async function fetchStars(synopsis){ 
  const response = await openai.createCompletion({
    model: 'text-davinci-003',
    max_tokens: 30,
    prompt:  `Extract the list of actors in brackets which are in the ${synopsis}. Separate each of them by a comma`
  })

  document.getElementById('output-stars').innerHTML = response.data.choices[0].text
}

async function fetchImagePromt(title, synopsis){
  const response = await openai.createCompletion({
    model: 'text-davinci-003',
    prompt: `Generate an image for a movie based on the ${title} with the synopsis ${synopsis}
      ###
